...
//Operator Overloading based on birthdate for users
bool User::operator<(User* anotherUser)
{
	if (comparedTo(anotherUser) == LESS) return true;
	else return false;
}
bool User::operator>(User* anotherUser)
{
	if (comparedTo(anotherUser) == GREATER) return true;
	else return false;
}
bool User::operator==(User* anotherUser)
{
	if (comparedTo(anotherUser) == EQUAL) return true;
	else return false;
}
